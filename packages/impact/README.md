...
A package containing the Impact theme design tokens
...
```bash
pnpm add -D @storm-themes/impact
```
...
```bash
npm install -D @storm-themes/impact
```
...
```bash
yarn add -D @storm-themes/impact
```
...
Run `nx build impact` to build the library.

### Running unit tests

Run `nx test impact` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint impact` to run [ESLint](https://eslint.org/) on the package.
